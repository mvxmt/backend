...
class DatabaseChunkManager:
    ...
    async def insert_chunk(self, document_id : int, chunk_text: str, chunk_vector: np.array):
        ...
        async with self.__conn.cursor() as cur:
            await cur.execute(insert_query,(document_id,chunk_text, chunk_vector,)
            )
    ...
    async def get_related_chunks(self, vector: np.array):
        select_query =  """
                        ...
                        """
        async with self.__conn.cursor() as cur:
            await cur.execute(select_query (vector,))
