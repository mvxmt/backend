...
class TestQueryManager(unittest.TestCase):
    ...
    def test_embed_user_prompt(self):
        ...
        embedding = ollama.embed(model=self.embedding_model, input=f"search_query: testing the embedding")
    ...
    def test_get_document_chunk(self):
        ...
        # Simulated results
        result = [
            (1, 1, 'the sky is blue', [.1,.2,.3]), 
            (2, 2, 'hello there how are you doing?', [.3,.4,.5]),
            (3, 3, 'the quick brown fox', [.2,.3,.4,.6])
            (4, 4, 'python is a myth', [.04,.03,.54])
        ]
